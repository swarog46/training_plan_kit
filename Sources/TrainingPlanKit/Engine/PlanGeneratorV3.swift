...
class PlanGeneratorV3 {
    ...
    static func make(config: PlanConfiguration, totalWeeks: Int, allWorkouts: [Workout], adaptive: Bool) -> PlanGeneratorV3 {
        // Maintenance plans have distance 0 and keep the base path for now.
        if config.runnerLevel == .beginner && config.distance != 0 {
            return BeginnerPlanGenerator(config: config, totalWeeks: totalWeeks, allWorkouts: allWorkouts, adaptive: adaptive)
        }
        return PlanGeneratorV3(config: config, totalWeeks: totalWeeks, allWorkouts: allWorkouts, adaptive: adaptive)
    }
